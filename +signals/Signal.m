classdef Signal < handle
    ...
    % Signals handle exceptions that can occur in the callback Ц functions
    ...
    methods
        ...
        function self = Signal(varargin)
            ...
            %
            % Parameters:
            %   sender: SenderТs object @type Sender
        ...
        function setSender(self, sender, name)
            ...
            % be added to the senderТs instance as a dynamic field under
            % the name "name".
            %
            % Usage::
            % @code
            ...
            %
            % Parameters:
            %   sender: SenderТs object shall be inherited from Sender 
            %   name: Field name in the senderТs instance, signal shall be added to this field
            %
            % @sa removeSender
            %
            ...
            if ~isempty(name)
                ...
                if isempty(prop)
                    % ≈сли пол€ нет, добавл€ем и устанавливаем в его 
                    % значение сигнал.
                    self.senderProp = sender.addprop(name);
                    sender.(name) = self;
                else
                    % ≈сли поле уже есть, то генерируем исключение если это
                    % поле не €вл€етс€ нашим сигналом. Ќельз€ создавать
    ...
    methods (Access = private)
        ...
        function receiveExceptionHandler(self, exception, receiver)
            % Exception handling in the receiverТs callback function
